...
# Intuition: sum up the values as you recurse down the tree
# Implementation: recursion + list to keep track of sums
# Time complexity: O(n)
# Space complexity: O(n)
def max_path_sum(root: Tree) -> int | float:
    sums = []

    def f(tree: Tree, total_sum: int | float):
        if is_leaf(tree):
            sums.append(total_sum + tree.label)
        else:
            for branch in tree.branches:
                f(branch, total_sum + tree.label)
